...
int My_DivC(int a, int b){
  int c = 0;
  asm(
    ".syntax unified\n\t"
    "my_div:\n"
    "Adds %[COCIENTE], %[COCIENTE], #1\n"
    "Subs %[DIVIDENDO], %[DIVIDENDO], %[DIVISOR]\n"
    "Bpl my_div\n"
    "Subs %[COCIENTE], %[COCIENTE], #1\n"
...
void lcd_display_dec(uint16_t value)
{
  if (value > 9999) {
    ...
    int div1000 = My_DivC(value, 1000);
    int div100 = My_DivC(value, 100);
    int div10 = My_DivC(value, 10);
    
    lcd_set(div1000, 1);
    
    int a = (value - (div1000)*1000);
    lcd_set(My_DivC(a,100), 2);
    
    int b = (value - (div100)*100);
    lcd_set(My_DivC(b,10), 3);
    
    lcd_set(value - (div10)*10, 4);
  }
}

extern int My_Div(int a, int b, int c);

void lcd_display_dec2(uint16_t value)
{
  if (value > 9999) {
    lcd_display_error(0x10);
  }else {
    int div1000 = My_Div(value, 1000, 0);
    int div100 = My_Div(value, 100, 0);
    int div10 = My_Div(value, 10, 0);
    
    lcd_set(div1000, 1);
    
    int a = (value - (div1000)*1000);
    lcd_set(My_Div(a,100,0), 2);
    
    int b = (value - (div100)*100);
    lcd_set(My_Div(b,10,0), 3);
